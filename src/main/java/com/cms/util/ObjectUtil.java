...
public class ObjectUtil {
    ...
    /**
	 * 比较一个对象下的两个不同实例中的所有属性，返回属性值不同的属性名称.
	 * @param clazz
	 * 			对象类
	 * @param objOne
	 * 			实例1
	 ...
	 * 			实例2
	 * @return
	 * 			map<属性名,Map<[valueOne][valueTwo],value>>
	 */
	public static Map<String, Map<String, String>> getObjctDefratAttr(Class clazz, Object objOne, Object objTwo) {

		/*如果两个对象有一个不是制定类型，则不进行比较，并返回null
		if( !(objOne.getClass().isAssignableFrom(clazz) && objTwo.getClass().isAssignableFrom(clazz)))
			return null;
		*/
		//属性对应的方法.
		Method metd = null;
		//属性名称.
		String fdname = null;
		Map<String, Map<String, String>> defratAttr = new HashMap<String, Map<String, String>>();
		try {
			// 获取集合中的对象类型
			Field[] fds = clazz.getDeclaredFields();// 获取他的字段数组
			for (Field field : fds) {// 遍历该数组
				if(!isBaseVariate(field.getType())) continue;
				fdname = field.getName();// 得到字段名
				if("SerialVersionUID".equalsIgnoreCase(fdname)) {
					continue;
				}
				metd = clazz.getMethod("get" + firstLetterToUpper(fdname), null);// 根据字段名找到对应的get方法，null表示无参数
				Object value1 = null != objOne ? metd.invoke(objOne, null):"";
				Object value2 = null != objTwo ? metd.invoke(objTwo, null):"";
				...
				if (	(null != value1 || null != value2) 
						...
						&& (!"".equals(value1) || null != value2)
						&& (null != value1 || !"".equals(value2))
						&& !(null != value1 && null != value2 && value1.equals(value2))) {
					Map<String, String> valMap = new HashMap<String, String>();
					if(field.getType() == Date.class){
						if(null != value1 && !"".equals(value1))
							valMap.put("valueOne", DateUtil.getWebDateString((Date)value1));
						else
							valMap.put("valueOne", "");
						if(null != value2 && !"".equals(value2))
							valMap.put("valueTwo", DateUtil.getWebDateString((Date)value2));
						else
							valMap.put("valueTwo", "");

					}else{
						if(null != value1)
							valMap.put("valueOne", value1.toString());
						else
							valMap.put("valueOne", "");
						
						if(null != value2)
							valMap.put("valueTwo", value2.toString());
						else
							valMap.put("valueTwo", "");
					}
					defratAttr.put(fdname, valMap);
				}
			}
	...
	public static boolean isBaseVariate(Class<?>  clazz) {
		boolean flag = true;
		if(flag && clazz == Integer.class)flag = false;
		if(flag && clazz == int.class)flag = false;
		if(flag && clazz == Long.class)flag = false;
		if(flag && clazz == long.class)flag = false;
		if(flag && clazz == Date.class)flag = false;
		if(flag && clazz == String.class)flag = false;
		if(flag && clazz == BigDecimal.class)flag = false;
		if(flag && clazz == Character.class)flag = false;
		if(flag && clazz == char.class)flag = false;
		if(flag && clazz == Double.class)flag = false;
		if(flag && clazz == double.class)flag = false;
		if(flag && clazz == Float.class)flag = false;
		if(flag && clazz == float.class)flag = false;
		if(flag && clazz == Short.class)flag = false;
		if(flag && clazz == short.class)flag = false;
		if(flag && clazz == Byte.class)flag = false;
		if(flag && clazz == byte.class)flag = false;
		return !flag;
	}
